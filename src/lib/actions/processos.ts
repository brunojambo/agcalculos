...
type StatusProcesso = typeof STATUS_VALIDOS[number];
...
function parseStatus(value: FormDataEntryValue | null): StatusProcesso {
  const status = String(value ?? "");
  if (!STATUS_VALIDOS.includes(status as StatusProcesso)) throw new Error("Status invalido.");
...
export async function criarProcesso(formData: FormData) {
  ...
  if (!processoId) throw new Error("Identificador ausente.");
  if (!reclamante || !clienteId || !tipoCalculoId) throw new Error("Campos obrigatorios ausentes.");
  ...
  await prisma.$transaction(async (tx: any) => {
    ...
    await tx.movimentacao.create({
      data: { processoId: processo.id, usuarioId, statusAnterior: null, statusNovo: processo.status, observacao: "Processo cadastrado." },
    });
  });
...
export async function alterarStatus(processoId: string, formData: FormData) {
  ...
  await prisma.$transaction([
    prisma.processo.update({
      where: { id: processoId },
      data: { status: statusNovo, dataEntrega: statusNovo === "FINALIZADO" ? new Date() : processo.dataEntrega },
    }),
    prisma.movimentacao.create({
      data: { processoId, usuarioId, statusAnterior: processo.status, statusNovo, observacao },
    }),
  ]);
